//
// Created by Porridge on 2023/9/16.
//
#include "TOFCorrection.h"

int main() {
    cv::Mat tof_data = (cv::Mat_<int>(8, 8) << 1127, 1062, 1036, 977, 976, 993, 1013, 1092,
            776, 746, 706, 687, 716, 694, 719, 749,
            588, 564, 544, 539, 540, 547, 553, 566,
            490, 480, 478, 479, 471, 475, 470, 477,
            433, 426, 425, 423, 423, 418, 420, 420,
            381, 380, 378, 378, 377, 373, 370, 370,
            338, 341, 341, 339, 338, 334, 334, 328,
            304, 306, 304, 305, 303, 302, 298, 297
    );
    cv::Vec3d rotation_vector_tof2wall(0.856371, -0.0144553, 0);
    cv::Vec3d gravity_standard(-0.130484,10.228030,0.477244);
    cv::Vec3d gravity_using(-0.120109,10.200096,0.497595);
    double EOF_angle = 24.12;
    double throw_ratio = 0.26;
    TOFCorrection correction(tof_data, rotation_vector_tof2wall,
                             gravity_standard, gravity_using, EOF_angle,
                             throw_ratio, true);
    cv::Mat offsets;
    correction.correct();
    offsets = correction.offsets;
    correction.update(tof_data, gravity_using);
    offsets = correction.offsets;
}