...
def main():
    ...
    chess_shape = (11, 8)
    chess_block_size = 30  # mm
    gc_step = 16
    black_thr = 40
    white_thr = 5

    camera_param_file = "camera_config.json"

    dirnames = sorted(glob.glob('./capture/capture_*'))
    if len(dirnames) == 0:
...
def calibrate(dirnames, gc_fname_lists, proj_shape, chess_shape, chess_block_size, gc_step, black_thr, white_thr, camP,
              camD):
    ...
    for dname, gc_filenames in zip(dirnames, gc_fname_lists):
        ...
        res, cam_corners = cv2.findChessboardCorners(white_img, chess_shape, cv2.CALIB_CB_ADAPTIVE_THRESH)
        if not res:
